...
public class VisionAlignCommand extends Command {
	...
	private final double INIT_DISTANCE = 12; // inches; initial distance from
												// the peg
	...
	protected void execute() {
		...
		if (Math.abs(anglePOV) > ANGLE_BUFFER) {
			// rotate first
			gyroTurnCommand = new GyroTurnCommand(
					(Math.abs(anglePOV) - ANGLE_BUFFER) * (anglePOV / -Math.abs(anglePOV)));
